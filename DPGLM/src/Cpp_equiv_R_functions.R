...
crm_sampler1 <- function(M, u, zstar, nstar, RL, RJ, tht, alpha) {
  N <- 3001
  R <- 3001
  eps <- 1e-6
  ...
  # Compute psi(z; u, tht) for all z at once
  psi_z <- rowSums(outer(u, z, function(u_i, z_i) u_i * exp(tht * z_i - max(tht * z_i))))
  
  # First Part: random locations and jumps
  fnS <- colMeans(exp(-(1 + psi_z) %o% s) / s) 
  ...
  for (m in seq_len(M)) {
    xi <- runif(1)
    temp <- exp(-(1 + psi_z) * RJ[m]) 
    cutoff <- sum(temp) * xi
    RL[m] <- z[min(which(cumsum(temp) > cutoff))]
  }
  
  # Second Part: random jumps [for fixed locations]
  psi_star <- rowSums(outer(u, zstar, function(u_i, zstar_i) u_i * exp(tht * zstar_i - max(tht * zstar_i))))
...
crm_sampler2 <- function(M, u, zstar, nstar, RL, RJ, tht, alpha) {
  N <- 3001
  R <- 3001
  eps <- 1e-6
  s <- -log(seq(exp(-eps), exp(-5e-4), length.out = N))
  ...
  # Second Part: random jumps [for fixed locations]
  psi_star <- rowSums(outer(u, zstar, function(u_i, zstar_i) u_i * exp(tht * zstar_i)))
